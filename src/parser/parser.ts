...
export class BlocksParser extends EmbeddedActionsParser {
  // Declare rule methods as properties (with !)
  public document!: () => (BlockNode | InlineNode | TextNode)[];
  private blockElement!: () => BlockNode;
  private commentBlock!: () => CommentBlockNode;
  private codeBlock!: () => CodeBlockNode;
  private scriptBlock!: () => ScriptBlockNode;
  private genericBlock!: () => GenericBlockNode;
  private inlineElement!: () => InlineNode;
  private commentInline!: () => CommentInlineNode;
  private codeInline!: () => CodeInlineNode;
  private scriptInline!: () => ScriptInlineNode;
  private genericInline!: () => GenericInlineNode;
  private textElement!: () => TextNode;
  ...
  constructor() {
    ...
    // Define all rules INSIDE the constructor
    
    // Main document rule
    this.document = this.RULE('document', (): (BlockNode | InlineNode | TextNode)[] => {
    ...
    // Block elements
    this.blockElement = this.RULE('blockElement', (): BlockNode => {
        return this.OR([
    ...
    this.commentBlock = this.RULE('commentBlock', (): CommentBlockNode => {
      ...
      // Try to consume name (first identifier after whitespace)
      ...
      // Try to consume attributes if present
    ...
    this.codeBlock = this.RULE('codeBlock', (): CodeBlockNode => {
    ...
    this.scriptBlock = this.RULE('scriptBlock', (): ScriptBlockNode => {
    ...
    this.genericBlock = this.RULE('genericBlock', (): GenericBlockNode => {
    ...
    // Inline elements
    this.inlineElement = this.RULE('inlineElement', (): InlineNode => {
      return this.OR([
    ...
    this.commentInline = this.RULE('commentInline', (): CommentInlineNode => {
    ...
    this.codeInline = this.RULE('codeInline', (): CodeInlineNode => {
    ...
    this.scriptInline = this.RULE('scriptInline', (): ScriptInlineNode => {
    ...
    this.genericInline = this.RULE('genericInline', (): GenericInlineNode => {
    ...
    // Text element
    this.textElement = this.RULE('textElement', (): TextNode => {
      const token = this.OR([
        { ALT: () => this.CONSUME(tokens.Content) },
        { ALT: () => this.CONSUME(tokens.Whitespace) },
        { ALT: () => this.CONSUME(tokens.Newline) },
    ...
    this.attributes = this.RULE('attributes', (): Attributes => {
